...
Token Lexer::next_token()
{
    ...
    if (is_ascii_alphanumeric(c) || c == '_') {
        ...
        if (char const first = m_lexer.peek(); !is_ascii_alphanumeric(first) && first != '_') {
            return Token { InvalidToken { "Invalid name start character"_string }, start_pos, start_line, start_column };
        }
    ...
    case ':':
        ...
        if (m_lexer.peek() == ':') {
            m_lexer.consume();
            m_current_column++;
            return Token { InvalidToken { "Unexpected '::' operator"_string }, start_pos, start_line, start_column };
        }
    ...
    return Token { InvalidToken { MUST(String::formatted("Invalid token encountered: {}"sv, unknown)) }, start_pos, start_line, start_column };
}
...
Token Lexer::tokenize_integer_literal()
{
    ...
    if (!is_ascii_digit(first)) {
        return Token { InvalidToken { "Invalid integer literal start character"_string }, start_pos, start_line, start_column };
    }
    ...
    if (first != '0') {
        ...
        return Token { InvalidToken { "Leading zero in integer literal is not allowed"_string }, start_pos, start_line, start_column };
    }
...
Token Lexer::tokenize(StringView const text, size_t const start_pos, size_t const start_line, size_t const start_column)
{
    ...
    if (text == "_") {
        return Token { InvalidToken { "Single underscore is not a valid identifier"_string }, start_pos, start_line, start_column };
    }
    if (text.to_byte_string().starts_with("__"sv)) {
        return Token { InvalidToken { "Identifiers cannot start with double underscore"_string }, start_pos, start_line, start_column };
    }
...
Token Lexer::tokenize_attribute(StringView const name, size_t const start_pos, size_t const start_line, size_t const start_column)
{
    ...
    if (m_lexer.peek() == '(') {
        ...
        if (paren_level > 0) {
            return Token { InvalidToken { "Unclosed attribute argument parentheses"_string }, start_pos, start_line, start_column };
        }
        ...
        if (name == "builtin") {
            ...
            return Token { InvalidToken { "Invalid builtin attribute argument"_string }, start_pos, start_line, start_column };
        }
        if (name == "location") {
            auto maybe_value = arg.to_number<u32>();
            if (!maybe_value.has_value()) {
                return Token { InvalidToken { "Invalid location attribute argument"_string }, start_pos, start_line, start_column };
            }
            return Token { AttributeToken { LocationAttribute { maybe_value.value() } }, start_pos, start_line, start_column };
        }
        return Token { InvalidToken { "Invalid attribute with arguments"_string }, start_pos, start_line, start_column };
    }
    ...
    return Token { InvalidToken { "Invalid attribute name"_string }, start_pos, start_line, start_column };
}
...
String Token::to_string() const
{
    ...
    type.visit(
        [&](InvalidToken const&) {
            builder.append("Invalid"_string);
        },
