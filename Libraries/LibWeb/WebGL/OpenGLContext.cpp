...
#    elif defined(USE_VULKAN_DMABUF_IMAGES) || defined(AK_OS_WINDOWS)
...
{
    ByteString const extensions(eglQueryString(m_impl->display, EGL_EXTENSIONS));
    if (!extensions.contains("EGL_ANGLE_d3d_share_handle_client_buffer"sv)) {
    ...
    auto maybe_shared_handle = d3d11_shared_texture->shared_handle();
    if (maybe_shared_handle.is_error()) {
        dbgln("GetSharedHandle failed: {}", maybe_shared_handle.error());
        return;
    }

    HANDLE shared_handle = maybe_shared_handle.release_value();

    winrt::com_ptr<ID3D12Resource> d3d12_resource;
    if (HRESULT const hr = d3d_ctx.d12_device().OpenSharedHandle(shared_handle, IID_PPV_ARGS(d3d12_resource.put())); FAILED(hr)) {
