...
JS::ThrowCompletionOr<HTMLCanvasElement::HasOrCreatedContext> HTMLCanvasElement::create_webgpu_context(JS::Value)
{
    if (!m_context.has<Empty>())
        return m_context.has<GC::Ref<WebGPU::GPUCanvasContext>>() ? HasOrCreatedContext::Yes : HasOrCreatedContext::No;

    m_context = TRY(WebGPU::GPUCanvasContext::create(realm(), *this));
    return HasOrCreatedContext::Yes;
}
...
void HTMLCanvasElement::present()
{
    ...
    m_context.visit(
        ...
        [](GC::Ref<WebGPU::GPUCanvasContext>&) {
            // Do nothing, GPUCC writes directly to the canvas surface.
        },
        [](Empty) {
            // Do nothing.
        });
