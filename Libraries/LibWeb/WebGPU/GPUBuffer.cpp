...
JS::ThrowCompletionOr<GC::Ref<GPUBuffer>> GPUBuffer::create(JS::Realm& realm, wgpu::Buffer buffer)
{
    ...
    return realm.create<GPUBuffer>(realm, Impl { .buffer = move(buffer), .size = size, .usage = usage, .map_state = map_state });
}
