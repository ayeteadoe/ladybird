...
struct GPUDeviceDescriptor : GPUObjectDescriptorBase {
    ...
    wgpu::DeviceDescriptor to_wgpu() const;
};
...
class GPUDevice final : public DOM::EventTarget {
    ...
    static JS::ThrowCompletionOr<GC::Ref<GPUDevice>> create(JS::Realm&, wgpu::Instance, wgpu::Device, String const&);

    ~GPUDevice() override;

    wgpu::Device wgpu() const;

    wgpu::Instance wgpu_instance() const;
