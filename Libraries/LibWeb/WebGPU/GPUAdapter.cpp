...
GC::Ref<WebIDL::Promise> GPUAdapter::request_device(GPUDeviceDescriptor const& options)
{
    ...
    Platform::EventLoopPlugin::the().deferred_invoke(GC::create_function(realm.heap(), [this, adapter, options, &realm, promise]() mutable {
        ...
        m_impl->instance.WaitAny(adapter.RequestDevice(&device_descriptor_options, wgpu::CallbackMode::AllowProcessEvents, [this, options, realm = GC::Root(realm), promise = GC::Root(promise)](wgpu::RequestDeviceStatus status, wgpu::Device native_device, char const* message) {
