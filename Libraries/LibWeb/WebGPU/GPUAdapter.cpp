...
GC::Ref<WebIDL::Promise> GPUAdapter::request_device(GPUDeviceDescriptor const& options)
{
    ...
    Platform::EventLoopPlugin::the().deferred_invoke(GC::create_function(realm.heap(), [this, adapter, options, &realm, promise]() mutable {
        auto const queue_label_view = options.default_queue.label.bytes_as_string_view();
...
// FIXME: Use wgpu::FeatureName::SharedTextureMemoryDXGISharedHandle for DirectX-based SkiaBackendContext on Windows
        ...
        m_impl->instance->wgpu().WaitAny(adapter.RequestDevice(&device_descriptor, wgpu::CallbackMode::AllowProcessEvents, [this, options, realm = GC::Root(realm), promise = GC::Root(promise)](wgpu::RequestDeviceStatus status, wgpu::Device native_device, char const* message) {
