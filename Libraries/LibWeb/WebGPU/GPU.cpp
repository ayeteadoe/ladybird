...
GC::Ref<WebIDL::Promise> GPU::request_adapter(GPURequestAdapterOptions const& options)
{
    ...
    auto callback = [&realm, promise](wgpu::RequestAdapterStatus status, wgpu::Adapter adapter, char const* message) {
    ...
    [[maybe_unused]] auto fut = m_impl->instance.RequestAdapter(&request_adapter_options, callbackMode, callback);
