...
// Enable WebGPU if we're on MacOS and can use Metal or if we can use shareable Vulkan images
...
void GPUCanvasContext::allocate_painting_surface_if_needed()
{
    ...
    auto vulkan_image = MUST(Gfx::create_shared_vulkan_image(m_impl->skia_backend_context->vulkan_context(), m_impl->size.width(), m_impl->size.height(), vulkan_format, 0, nullptr));
    m_impl->drawing_buffer = Gfx::PaintingSurface::create_from_vkimage(m_impl->skia_backend_context, vulkan_image, Gfx::PaintingSurface::Origin::BottomLeft);
    wgpu::SharedTextureMemoryDmaBufPlane shared_texture_memory_dma_buf_plane {};
    shared_texture_memory_dma_buf_plane.fd = vulkan_image->get_dma_buf_fd();
    shared_texture_memory_dma_buf_plane.offset = 0;
    shared_texture_memory_dma_buf_plane.stride = vulkan_image->info.row_pitch;
    wgpu::SharedTextureMemoryDmaBufDescriptor shared_texture_memory_dma_buf_descriptor {};
    shared_texture_memory_dma_buf_descriptor.drmFormat = Gfx::vk_format_to_drm_format(vulkan_format);
    shared_texture_memory_dma_buf_descriptor.drmModifier = vulkan_image->info.modifier;
    shared_texture_memory_dma_buf_descriptor.planeCount = 1;
    shared_texture_memory_dma_buf_descriptor.planes = &shared_texture_memory_dma_buf_plane;
    shared_texture_memory_descriptor.nextInChain = &shared_texture_memory_dma_buf_descriptor;
